...
Item {
  ...
  // https://github.com/basecamp/omarchy/issues/8007, fixed upstream (via
  // Loader.initialProperties) by
  ...
  // Optional "#RGB"/"#RRGGBB" override from shell.json's `bar:` entry.
  // Empty (the default, and what an absent or malformed value falls back to)
  // means "leave the theme in charge", so the bar keeps following theme
  // switches exactly as it does stock.
  ...
  function applyBarConfig() {
    ...
    marginGap = typeof config.marginGap === "number"
      ? Math.max(0, Math.min(200, config.marginGap)) : 0
    configuredCornerRadius = typeof config.cornerRadius === "number"
      ? Math.max(0, Math.min(200, config.cornerRadius)) : -1
    barOpacity = typeof config.opacity === "number"
    ...
    if (bg !== "" && !/^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(bg)) {
      console.warn("bar: ignoring invalid background " + JSON.stringify(bg)
        + " -- expected \"#RGB\" or \"#RRGGBB\"; using the theme color")
      bg = ""
    }
  ...
  component BarPanel: PanelWindow {
    ...
    function marginFor(edge) {
      return edge === innerEdge() ? 0 : root.marginGap
    }
